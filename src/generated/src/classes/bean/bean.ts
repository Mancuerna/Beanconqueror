// Code generated by protoc-gen-ts_proto. DO NOT EDIT.
// versions:
//   protoc-gen-ts_proto  v2.11.6
//   protoc               v5.29.3
// source: src/classes/bean/bean.proto

/* eslint-disable */
import { BinaryReader, BinaryWriter } from '@bufbuild/protobuf/wire';

export const protobufPackage = 'beanconqueror';

export enum Roast {
  /** UNKNOWN_ROAST - UNKOWN */
  UNKNOWN_ROAST = 0,
  /** CINNAMON_ROAST - LIGHT */
  CINNAMON_ROAST = 1,
  AMERICAN_ROAST = 2,
  NEW_ENGLAND_ROAST = 3,
  HALF_CITY_ROAST = 4,
  MODERATE_LIGHT_ROAST = 5,
  /** CITY_ROAST - MEDIUM */
  CITY_ROAST = 6,
  CITY_PLUS_ROAST = 7,
  FULL_CITY_ROAST = 8,
  /** FULL_CITY_PLUS_ROAST - DARK */
  FULL_CITY_PLUS_ROAST = 9,
  ITALIAN_ROAST = 10,
  VIEANNA_ROAST = 11,
  FRENCH_ROAST = 12,
  CUSTOM_ROAST = 13,
  UNRECOGNIZED = -1,
}

export function roastFromJSON(object: any): Roast {
  switch (object) {
    case 0:
    case 'UNKNOWN_ROAST':
      return Roast.UNKNOWN_ROAST;
    case 1:
    case 'CINNAMON_ROAST':
      return Roast.CINNAMON_ROAST;
    case 2:
    case 'AMERICAN_ROAST':
      return Roast.AMERICAN_ROAST;
    case 3:
    case 'NEW_ENGLAND_ROAST':
      return Roast.NEW_ENGLAND_ROAST;
    case 4:
    case 'HALF_CITY_ROAST':
      return Roast.HALF_CITY_ROAST;
    case 5:
    case 'MODERATE_LIGHT_ROAST':
      return Roast.MODERATE_LIGHT_ROAST;
    case 6:
    case 'CITY_ROAST':
      return Roast.CITY_ROAST;
    case 7:
    case 'CITY_PLUS_ROAST':
      return Roast.CITY_PLUS_ROAST;
    case 8:
    case 'FULL_CITY_ROAST':
      return Roast.FULL_CITY_ROAST;
    case 9:
    case 'FULL_CITY_PLUS_ROAST':
      return Roast.FULL_CITY_PLUS_ROAST;
    case 10:
    case 'ITALIAN_ROAST':
      return Roast.ITALIAN_ROAST;
    case 11:
    case 'VIEANNA_ROAST':
      return Roast.VIEANNA_ROAST;
    case 12:
    case 'FRENCH_ROAST':
      return Roast.FRENCH_ROAST;
    case 13:
    case 'CUSTOM_ROAST':
      return Roast.CUSTOM_ROAST;
    case -1:
    case 'UNRECOGNIZED':
    default:
      return Roast.UNRECOGNIZED;
  }
}

export function roastToJSON(object: Roast): string {
  switch (object) {
    case Roast.UNKNOWN_ROAST:
      return 'UNKNOWN_ROAST';
    case Roast.CINNAMON_ROAST:
      return 'CINNAMON_ROAST';
    case Roast.AMERICAN_ROAST:
      return 'AMERICAN_ROAST';
    case Roast.NEW_ENGLAND_ROAST:
      return 'NEW_ENGLAND_ROAST';
    case Roast.HALF_CITY_ROAST:
      return 'HALF_CITY_ROAST';
    case Roast.MODERATE_LIGHT_ROAST:
      return 'MODERATE_LIGHT_ROAST';
    case Roast.CITY_ROAST:
      return 'CITY_ROAST';
    case Roast.CITY_PLUS_ROAST:
      return 'CITY_PLUS_ROAST';
    case Roast.FULL_CITY_ROAST:
      return 'FULL_CITY_ROAST';
    case Roast.FULL_CITY_PLUS_ROAST:
      return 'FULL_CITY_PLUS_ROAST';
    case Roast.ITALIAN_ROAST:
      return 'ITALIAN_ROAST';
    case Roast.VIEANNA_ROAST:
      return 'VIEANNA_ROAST';
    case Roast.FRENCH_ROAST:
      return 'FRENCH_ROAST';
    case Roast.CUSTOM_ROAST:
      return 'CUSTOM_ROAST';
    case Roast.UNRECOGNIZED:
    default:
      return 'UNRECOGNIZED';
  }
}

export enum BeanMix {
  UNKNOWN_BEAN_MIX = 0,
  SINGLE_ORIGIN = 1,
  BLEND = 2,
  UNRECOGNIZED = -1,
}

export function beanMixFromJSON(object: any): BeanMix {
  switch (object) {
    case 0:
    case 'UNKNOWN_BEAN_MIX':
      return BeanMix.UNKNOWN_BEAN_MIX;
    case 1:
    case 'SINGLE_ORIGIN':
      return BeanMix.SINGLE_ORIGIN;
    case 2:
    case 'BLEND':
      return BeanMix.BLEND;
    case -1:
    case 'UNRECOGNIZED':
    default:
      return BeanMix.UNRECOGNIZED;
  }
}

export function beanMixToJSON(object: BeanMix): string {
  switch (object) {
    case BeanMix.UNKNOWN_BEAN_MIX:
      return 'UNKNOWN_BEAN_MIX';
    case BeanMix.SINGLE_ORIGIN:
      return 'SINGLE_ORIGIN';
    case BeanMix.BLEND:
      return 'BLEND';
    case BeanMix.UNRECOGNIZED:
    default:
      return 'UNRECOGNIZED';
  }
}

export enum BeanRoastingType {
  UNKNOWN_BEAN_ROASTING_TYPE = 0,
  FILTER = 1,
  ESPRESSO = 2,
  OMNI = 3,
  UNRECOGNIZED = -1,
}

export function beanRoastingTypeFromJSON(object: any): BeanRoastingType {
  switch (object) {
    case 0:
    case 'UNKNOWN_BEAN_ROASTING_TYPE':
      return BeanRoastingType.UNKNOWN_BEAN_ROASTING_TYPE;
    case 1:
    case 'FILTER':
      return BeanRoastingType.FILTER;
    case 2:
    case 'ESPRESSO':
      return BeanRoastingType.ESPRESSO;
    case 3:
    case 'OMNI':
      return BeanRoastingType.OMNI;
    case -1:
    case 'UNRECOGNIZED':
    default:
      return BeanRoastingType.UNRECOGNIZED;
  }
}

export function beanRoastingTypeToJSON(object: BeanRoastingType): string {
  switch (object) {
    case BeanRoastingType.UNKNOWN_BEAN_ROASTING_TYPE:
      return 'UNKNOWN_BEAN_ROASTING_TYPE';
    case BeanRoastingType.FILTER:
      return 'FILTER';
    case BeanRoastingType.ESPRESSO:
      return 'ESPRESSO';
    case BeanRoastingType.OMNI:
      return 'OMNI';
    case BeanRoastingType.UNRECOGNIZED:
    default:
      return 'UNRECOGNIZED';
  }
}

export interface BeanProto {
  name: string;
  buyDate?: string | undefined;
  roastingDate?: string | undefined;
  note?: string | undefined;
  roaster?: string | undefined;
  config?: Config | undefined;
  roast?: Roast | undefined;
  roast_range?: number | undefined;
  beanMix?: BeanMix | undefined;
  roast_custom?: string | undefined;
  aromatics?: string | undefined;
  weight?: number | undefined;
  finished?: boolean | undefined;
  cost?: number | undefined;
  attachments: string[];
  cupping_points?: string | undefined;
  decaffeinated?: boolean | undefined;
  url?: string | undefined;
  ean_article_number?: string | undefined;
  rating?: number | undefined;
  bean_information: BeanInformation[];
  bean_roasting_type?: BeanRoastingType | undefined;
  bean_roast_information?: BeanRoastInformation | undefined;
  qr_code?: string | undefined;
  favourite?: boolean | undefined;
  shared?: boolean | undefined;
  cupping?: ICupping | undefined;
  cupped_flavor?: IFlavor | undefined;
  external_images: string[];
}

export interface Config {
  uuid: string;
  unix_timestamp: number;
}

export interface BeanInformation {
  country?: string | undefined;
  region?: string | undefined;
  farm?: string | undefined;
  farmer?: string | undefined;
  elevation?: string | undefined;
  harvest_time?: string | undefined;
  variety?: string | undefined;
  processing?: string | undefined;
  certification?: string | undefined;
  percentage?: number | undefined;
  purchasing_price?: number | undefined;
  fob_price?: number | undefined;
}

export interface BeanRoastInformation {
  drop_temperature?: number | undefined;
  roast_length?: number | undefined;
  roaster_machine?: string | undefined;
  green_bean_weight?: number | undefined;
  outside_temperature?: number | undefined;
  humidity?: number | undefined;
  bean_uuid?: string | undefined;
  first_crack_minute?: number | undefined;
  first_crack_temperature?: number | undefined;
  second_crack_minute?: number | undefined;
  second_crack_temperature?: number | undefined;
}

export interface ICupping {
  dry_fragrance?: number | undefined;
  wet_aroma?: number | undefined;
  brightness?: number | undefined;
  flavor?: number | undefined;
  body?: number | undefined;
  finish?: number | undefined;
  sweetness?: number | undefined;
  clean_cup?: number | undefined;
  complexity?: number | undefined;
  uniformity?: number | undefined;
  cuppers_correction?: number | undefined;
}

export interface IFlavor {
  predefined_flavors: number[];
  custom_flavors: string[];
}

function createBaseBeanProto(): BeanProto {
  return {
    name: '',
    buyDate: undefined,
    roastingDate: undefined,
    note: undefined,
    roaster: undefined,
    config: undefined,
    roast: undefined,
    roast_range: undefined,
    beanMix: undefined,
    roast_custom: undefined,
    aromatics: undefined,
    weight: undefined,
    finished: undefined,
    cost: undefined,
    attachments: [],
    cupping_points: undefined,
    decaffeinated: undefined,
    url: undefined,
    ean_article_number: undefined,
    rating: undefined,
    bean_information: [],
    bean_roasting_type: undefined,
    bean_roast_information: undefined,
    qr_code: undefined,
    favourite: undefined,
    shared: undefined,
    cupping: undefined,
    cupped_flavor: undefined,
    external_images: [],
  };
}

export const BeanProto: MessageFns<BeanProto> = {
  encode(
    message: BeanProto,
    writer: BinaryWriter = new BinaryWriter(),
  ): BinaryWriter {
    if (message.name !== '') {
      writer.uint32(10).string(message.name);
    }
    if (message.buyDate !== undefined) {
      writer.uint32(18).string(message.buyDate);
    }
    if (message.roastingDate !== undefined) {
      writer.uint32(26).string(message.roastingDate);
    }
    if (message.note !== undefined) {
      writer.uint32(34).string(message.note);
    }
    if (message.roaster !== undefined) {
      writer.uint32(42).string(message.roaster);
    }
    if (message.config !== undefined) {
      Config.encode(message.config, writer.uint32(50).fork()).join();
    }
    if (message.roast !== undefined) {
      writer.uint32(56).int32(message.roast);
    }
    if (message.roast_range !== undefined) {
      writer.uint32(64).uint64(message.roast_range);
    }
    if (message.beanMix !== undefined) {
      writer.uint32(72).int32(message.beanMix);
    }
    if (message.roast_custom !== undefined) {
      writer.uint32(82).string(message.roast_custom);
    }
    if (message.aromatics !== undefined) {
      writer.uint32(90).string(message.aromatics);
    }
    if (message.weight !== undefined) {
      writer.uint32(97).double(message.weight);
    }
    if (message.finished !== undefined) {
      writer.uint32(104).bool(message.finished);
    }
    if (message.cost !== undefined) {
      writer.uint32(113).double(message.cost);
    }
    for (const v of message.attachments) {
      writer.uint32(122).string(v!);
    }
    if (message.cupping_points !== undefined) {
      writer.uint32(130).string(message.cupping_points);
    }
    if (message.decaffeinated !== undefined) {
      writer.uint32(136).bool(message.decaffeinated);
    }
    if (message.url !== undefined) {
      writer.uint32(146).string(message.url);
    }
    if (message.ean_article_number !== undefined) {
      writer.uint32(154).string(message.ean_article_number);
    }
    if (message.rating !== undefined) {
      writer.uint32(161).double(message.rating);
    }
    for (const v of message.bean_information) {
      BeanInformation.encode(v!, writer.uint32(170).fork()).join();
    }
    if (message.bean_roasting_type !== undefined) {
      writer.uint32(176).int32(message.bean_roasting_type);
    }
    if (message.bean_roast_information !== undefined) {
      BeanRoastInformation.encode(
        message.bean_roast_information,
        writer.uint32(186).fork(),
      ).join();
    }
    if (message.qr_code !== undefined) {
      writer.uint32(194).string(message.qr_code);
    }
    if (message.favourite !== undefined) {
      writer.uint32(200).bool(message.favourite);
    }
    if (message.shared !== undefined) {
      writer.uint32(208).bool(message.shared);
    }
    if (message.cupping !== undefined) {
      ICupping.encode(message.cupping, writer.uint32(218).fork()).join();
    }
    if (message.cupped_flavor !== undefined) {
      IFlavor.encode(message.cupped_flavor, writer.uint32(226).fork()).join();
    }
    for (const v of message.external_images) {
      writer.uint32(234).string(v!);
    }
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): BeanProto {
    const reader =
      input instanceof BinaryReader ? input : new BinaryReader(input);
    const end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseBeanProto();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1: {
          if (tag !== 10) {
            break;
          }

          message.name = reader.string();
          continue;
        }
        case 2: {
          if (tag !== 18) {
            break;
          }

          message.buyDate = reader.string();
          continue;
        }
        case 3: {
          if (tag !== 26) {
            break;
          }

          message.roastingDate = reader.string();
          continue;
        }
        case 4: {
          if (tag !== 34) {
            break;
          }

          message.note = reader.string();
          continue;
        }
        case 5: {
          if (tag !== 42) {
            break;
          }

          message.roaster = reader.string();
          continue;
        }
        case 6: {
          if (tag !== 50) {
            break;
          }

          message.config = Config.decode(reader, reader.uint32());
          continue;
        }
        case 7: {
          if (tag !== 56) {
            break;
          }

          message.roast = reader.int32() as any;
          continue;
        }
        case 8: {
          if (tag !== 64) {
            break;
          }

          message.roast_range = longToNumber(reader.uint64());
          continue;
        }
        case 9: {
          if (tag !== 72) {
            break;
          }

          message.beanMix = reader.int32() as any;
          continue;
        }
        case 10: {
          if (tag !== 82) {
            break;
          }

          message.roast_custom = reader.string();
          continue;
        }
        case 11: {
          if (tag !== 90) {
            break;
          }

          message.aromatics = reader.string();
          continue;
        }
        case 12: {
          if (tag === 96) {
            message.weight = longToNumber(reader.uint64());
            continue;
          }
          if (tag !== 97) {
            break;
          }

          message.weight = reader.double();
          continue;
        }
        case 13: {
          if (tag !== 104) {
            break;
          }

          message.finished = reader.bool();
          continue;
        }
        case 14: {
          if (tag === 112) {
            message.cost = longToNumber(reader.uint64());
            continue;
          }
          if (tag !== 113) {
            break;
          }

          message.cost = reader.double();
          continue;
        }
        case 15: {
          if (tag !== 122) {
            break;
          }

          message.attachments.push(reader.string());
          continue;
        }
        case 16: {
          if (tag !== 130) {
            break;
          }

          message.cupping_points = reader.string();
          continue;
        }
        case 17: {
          if (tag !== 136) {
            break;
          }

          message.decaffeinated = reader.bool();
          continue;
        }
        case 18: {
          if (tag !== 146) {
            break;
          }

          message.url = reader.string();
          continue;
        }
        case 19: {
          if (tag !== 154) {
            break;
          }

          message.ean_article_number = reader.string();
          continue;
        }
        case 20: {
          if (tag === 160) {
            message.rating = reader.uint32();
            continue;
          }
          if (tag !== 161) {
            break;
          }

          message.rating = reader.double();
          continue;
        }
        case 21: {
          if (tag !== 170) {
            break;
          }

          message.bean_information.push(
            BeanInformation.decode(reader, reader.uint32()),
          );
          continue;
        }
        case 22: {
          if (tag !== 176) {
            break;
          }

          message.bean_roasting_type = reader.int32() as any;
          continue;
        }
        case 23: {
          if (tag !== 186) {
            break;
          }

          message.bean_roast_information = BeanRoastInformation.decode(
            reader,
            reader.uint32(),
          );
          continue;
        }
        case 24: {
          if (tag !== 194) {
            break;
          }

          message.qr_code = reader.string();
          continue;
        }
        case 25: {
          if (tag !== 200) {
            break;
          }

          message.favourite = reader.bool();
          continue;
        }
        case 26: {
          if (tag !== 208) {
            break;
          }

          message.shared = reader.bool();
          continue;
        }
        case 27: {
          if (tag !== 218) {
            break;
          }

          message.cupping = ICupping.decode(reader, reader.uint32());
          continue;
        }
        case 28: {
          if (tag !== 226) {
            break;
          }

          message.cupped_flavor = IFlavor.decode(reader, reader.uint32());
          continue;
        }
        case 29: {
          if (tag !== 234) {
            break;
          }

          message.external_images.push(reader.string());
          continue;
        }
      }
      if ((tag & 7) === 4 || tag === 0) {
        break;
      }
      reader.skip(tag & 7);
    }
    return message;
  },

  fromJSON(object: any): BeanProto {
    return {
      name: isSet(object.name) ? globalThis.String(object.name) : '',
      buyDate: isSet(object.buyDate)
        ? globalThis.String(object.buyDate)
        : undefined,
      roastingDate: isSet(object.roastingDate)
        ? globalThis.String(object.roastingDate)
        : undefined,
      note: isSet(object.note) ? globalThis.String(object.note) : undefined,
      roaster: isSet(object.roaster)
        ? globalThis.String(object.roaster)
        : undefined,
      config: isSet(object.config) ? Config.fromJSON(object.config) : undefined,
      roast: isSet(object.roast) ? roastFromJSON(object.roast) : undefined,
      roast_range: isSet(object.roastRange)
        ? globalThis.Number(object.roastRange)
        : isSet(object.roast_range)
          ? globalThis.Number(object.roast_range)
          : undefined,
      beanMix: isSet(object.beanMix)
        ? beanMixFromJSON(object.beanMix)
        : undefined,
      roast_custom: isSet(object.roastCustom)
        ? globalThis.String(object.roastCustom)
        : isSet(object.roast_custom)
          ? globalThis.String(object.roast_custom)
          : undefined,
      aromatics: isSet(object.aromatics)
        ? globalThis.String(object.aromatics)
        : undefined,
      weight: isSet(object.weight)
        ? globalThis.Number(object.weight)
        : undefined,
      finished: isSet(object.finished)
        ? globalThis.Boolean(object.finished)
        : undefined,
      cost: isSet(object.cost) ? globalThis.Number(object.cost) : undefined,
      attachments: globalThis.Array.isArray(object?.attachments)
        ? object.attachments.map((e: any) => globalThis.String(e))
        : [],
      cupping_points: isSet(object.cuppingPoints)
        ? globalThis.String(object.cuppingPoints)
        : isSet(object.cupping_points)
          ? globalThis.String(object.cupping_points)
          : undefined,
      decaffeinated: isSet(object.decaffeinated)
        ? globalThis.Boolean(object.decaffeinated)
        : undefined,
      url: isSet(object.url) ? globalThis.String(object.url) : undefined,
      ean_article_number: isSet(object.eanArticleNumber)
        ? globalThis.String(object.eanArticleNumber)
        : isSet(object.ean_article_number)
          ? globalThis.String(object.ean_article_number)
          : undefined,
      rating: isSet(object.rating)
        ? globalThis.Number(object.rating)
        : undefined,
      bean_information: globalThis.Array.isArray(object?.beanInformation)
        ? object.beanInformation.map((e: any) => BeanInformation.fromJSON(e))
        : globalThis.Array.isArray(object?.bean_information)
          ? object.bean_information.map((e: any) => BeanInformation.fromJSON(e))
          : [],
      bean_roasting_type: isSet(object.beanRoastingType)
        ? beanRoastingTypeFromJSON(object.beanRoastingType)
        : isSet(object.bean_roasting_type)
          ? beanRoastingTypeFromJSON(object.bean_roasting_type)
          : undefined,
      bean_roast_information: isSet(object.beanRoastInformation)
        ? BeanRoastInformation.fromJSON(object.beanRoastInformation)
        : isSet(object.bean_roast_information)
          ? BeanRoastInformation.fromJSON(object.bean_roast_information)
          : undefined,
      qr_code: isSet(object.qrCode)
        ? globalThis.String(object.qrCode)
        : isSet(object.qr_code)
          ? globalThis.String(object.qr_code)
          : undefined,
      favourite: isSet(object.favourite)
        ? globalThis.Boolean(object.favourite)
        : undefined,
      shared: isSet(object.shared)
        ? globalThis.Boolean(object.shared)
        : undefined,
      cupping: isSet(object.cupping)
        ? ICupping.fromJSON(object.cupping)
        : undefined,
      cupped_flavor: isSet(object.cuppedFlavor)
        ? IFlavor.fromJSON(object.cuppedFlavor)
        : isSet(object.cupped_flavor)
          ? IFlavor.fromJSON(object.cupped_flavor)
          : undefined,
      external_images: globalThis.Array.isArray(object?.externalImages)
        ? object.externalImages.map((e: any) => globalThis.String(e))
        : globalThis.Array.isArray(object?.external_images)
          ? object.external_images.map((e: any) => globalThis.String(e))
          : [],
    };
  },

  toJSON(message: BeanProto): unknown {
    const obj: any = {};
    if (message.name !== '') {
      obj.name = message.name;
    }
    if (message.buyDate !== undefined) {
      obj.buyDate = message.buyDate;
    }
    if (message.roastingDate !== undefined) {
      obj.roastingDate = message.roastingDate;
    }
    if (message.note !== undefined) {
      obj.note = message.note;
    }
    if (message.roaster !== undefined) {
      obj.roaster = message.roaster;
    }
    if (message.config !== undefined) {
      obj.config = Config.toJSON(message.config);
    }
    if (message.roast !== undefined) {
      obj.roast = roastToJSON(message.roast);
    }
    if (message.roast_range !== undefined) {
      obj.roastRange = Math.round(message.roast_range);
    }
    if (message.beanMix !== undefined) {
      obj.beanMix = beanMixToJSON(message.beanMix);
    }
    if (message.roast_custom !== undefined) {
      obj.roastCustom = message.roast_custom;
    }
    if (message.aromatics !== undefined) {
      obj.aromatics = message.aromatics;
    }
    if (message.weight !== undefined) {
      obj.weight = message.weight;
    }
    if (message.finished !== undefined) {
      obj.finished = message.finished;
    }
    if (message.cost !== undefined) {
      obj.cost = message.cost;
    }
    if (message.attachments?.length) {
      obj.attachments = message.attachments;
    }
    if (message.cupping_points !== undefined) {
      obj.cuppingPoints = message.cupping_points;
    }
    if (message.decaffeinated !== undefined) {
      obj.decaffeinated = message.decaffeinated;
    }
    if (message.url !== undefined) {
      obj.url = message.url;
    }
    if (message.ean_article_number !== undefined) {
      obj.eanArticleNumber = message.ean_article_number;
    }
    if (message.rating !== undefined) {
      obj.rating = message.rating;
    }
    if (message.bean_information?.length) {
      obj.beanInformation = message.bean_information.map((e) =>
        BeanInformation.toJSON(e),
      );
    }
    if (message.bean_roasting_type !== undefined) {
      obj.beanRoastingType = beanRoastingTypeToJSON(message.bean_roasting_type);
    }
    if (message.bean_roast_information !== undefined) {
      obj.beanRoastInformation = BeanRoastInformation.toJSON(
        message.bean_roast_information,
      );
    }
    if (message.qr_code !== undefined) {
      obj.qrCode = message.qr_code;
    }
    if (message.favourite !== undefined) {
      obj.favourite = message.favourite;
    }
    if (message.shared !== undefined) {
      obj.shared = message.shared;
    }
    if (message.cupping !== undefined) {
      obj.cupping = ICupping.toJSON(message.cupping);
    }
    if (message.cupped_flavor !== undefined) {
      obj.cuppedFlavor = IFlavor.toJSON(message.cupped_flavor);
    }
    if (message.external_images?.length) {
      obj.externalImages = message.external_images;
    }
    return obj;
  },

  create<I extends Exact<DeepPartial<BeanProto>, I>>(base?: I): BeanProto {
    return BeanProto.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<BeanProto>, I>>(
    object: I,
  ): BeanProto {
    const message = createBaseBeanProto();
    message.name = object.name ?? '';
    message.buyDate = object.buyDate ?? undefined;
    message.roastingDate = object.roastingDate ?? undefined;
    message.note = object.note ?? undefined;
    message.roaster = object.roaster ?? undefined;
    message.config =
      object.config !== undefined && object.config !== null
        ? Config.fromPartial(object.config)
        : undefined;
    message.roast = object.roast ?? undefined;
    message.roast_range = object.roast_range ?? undefined;
    message.beanMix = object.beanMix ?? undefined;
    message.roast_custom = object.roast_custom ?? undefined;
    message.aromatics = object.aromatics ?? undefined;
    message.weight = object.weight ?? undefined;
    message.finished = object.finished ?? undefined;
    message.cost = object.cost ?? undefined;
    message.attachments = object.attachments?.map((e) => e) || [];
    message.cupping_points = object.cupping_points ?? undefined;
    message.decaffeinated = object.decaffeinated ?? undefined;
    message.url = object.url ?? undefined;
    message.ean_article_number = object.ean_article_number ?? undefined;
    message.rating = object.rating ?? undefined;
    message.bean_information =
      object.bean_information?.map((e) => BeanInformation.fromPartial(e)) || [];
    message.bean_roasting_type = object.bean_roasting_type ?? undefined;
    message.bean_roast_information =
      object.bean_roast_information !== undefined &&
      object.bean_roast_information !== null
        ? BeanRoastInformation.fromPartial(object.bean_roast_information)
        : undefined;
    message.qr_code = object.qr_code ?? undefined;
    message.favourite = object.favourite ?? undefined;
    message.shared = object.shared ?? undefined;
    message.cupping =
      object.cupping !== undefined && object.cupping !== null
        ? ICupping.fromPartial(object.cupping)
        : undefined;
    message.cupped_flavor =
      object.cupped_flavor !== undefined && object.cupped_flavor !== null
        ? IFlavor.fromPartial(object.cupped_flavor)
        : undefined;
    message.external_images = object.external_images?.map((e) => e) || [];
    return message;
  },
};

function createBaseConfig(): Config {
  return { uuid: '', unix_timestamp: 0 };
}

export const Config: MessageFns<Config> = {
  encode(
    message: Config,
    writer: BinaryWriter = new BinaryWriter(),
  ): BinaryWriter {
    if (message.uuid !== '') {
      writer.uint32(10).string(message.uuid);
    }
    if (message.unix_timestamp !== 0) {
      writer.uint32(16).uint64(message.unix_timestamp);
    }
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): Config {
    const reader =
      input instanceof BinaryReader ? input : new BinaryReader(input);
    const end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseConfig();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1: {
          if (tag !== 10) {
            break;
          }

          message.uuid = reader.string();
          continue;
        }
        case 2: {
          if (tag !== 16) {
            break;
          }

          message.unix_timestamp = longToNumber(reader.uint64());
          continue;
        }
      }
      if ((tag & 7) === 4 || tag === 0) {
        break;
      }
      reader.skip(tag & 7);
    }
    return message;
  },

  fromJSON(object: any): Config {
    return {
      uuid: isSet(object.uuid) ? globalThis.String(object.uuid) : '',
      unix_timestamp: isSet(object.unixTimestamp)
        ? globalThis.Number(object.unixTimestamp)
        : isSet(object.unix_timestamp)
          ? globalThis.Number(object.unix_timestamp)
          : 0,
    };
  },

  toJSON(message: Config): unknown {
    const obj: any = {};
    if (message.uuid !== '') {
      obj.uuid = message.uuid;
    }
    if (message.unix_timestamp !== 0) {
      obj.unixTimestamp = Math.round(message.unix_timestamp);
    }
    return obj;
  },

  create<I extends Exact<DeepPartial<Config>, I>>(base?: I): Config {
    return Config.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<Config>, I>>(object: I): Config {
    const message = createBaseConfig();
    message.uuid = object.uuid ?? '';
    message.unix_timestamp = object.unix_timestamp ?? 0;
    return message;
  },
};

function createBaseBeanInformation(): BeanInformation {
  return {
    country: undefined,
    region: undefined,
    farm: undefined,
    farmer: undefined,
    elevation: undefined,
    harvest_time: undefined,
    variety: undefined,
    processing: undefined,
    certification: undefined,
    percentage: undefined,
    purchasing_price: undefined,
    fob_price: undefined,
  };
}

export const BeanInformation: MessageFns<BeanInformation> = {
  encode(
    message: BeanInformation,
    writer: BinaryWriter = new BinaryWriter(),
  ): BinaryWriter {
    if (message.country !== undefined) {
      writer.uint32(10).string(message.country);
    }
    if (message.region !== undefined) {
      writer.uint32(18).string(message.region);
    }
    if (message.farm !== undefined) {
      writer.uint32(26).string(message.farm);
    }
    if (message.farmer !== undefined) {
      writer.uint32(34).string(message.farmer);
    }
    if (message.elevation !== undefined) {
      writer.uint32(42).string(message.elevation);
    }
    if (message.harvest_time !== undefined) {
      writer.uint32(50).string(message.harvest_time);
    }
    if (message.variety !== undefined) {
      writer.uint32(58).string(message.variety);
    }
    if (message.processing !== undefined) {
      writer.uint32(66).string(message.processing);
    }
    if (message.certification !== undefined) {
      writer.uint32(74).string(message.certification);
    }
    if (message.percentage !== undefined) {
      writer.uint32(81).double(message.percentage);
    }
    if (message.purchasing_price !== undefined) {
      writer.uint32(89).double(message.purchasing_price);
    }
    if (message.fob_price !== undefined) {
      writer.uint32(97).double(message.fob_price);
    }
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): BeanInformation {
    const reader =
      input instanceof BinaryReader ? input : new BinaryReader(input);
    const end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseBeanInformation();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1: {
          if (tag !== 10) {
            break;
          }

          message.country = reader.string();
          continue;
        }
        case 2: {
          if (tag !== 18) {
            break;
          }

          message.region = reader.string();
          continue;
        }
        case 3: {
          if (tag !== 26) {
            break;
          }

          message.farm = reader.string();
          continue;
        }
        case 4: {
          if (tag !== 34) {
            break;
          }

          message.farmer = reader.string();
          continue;
        }
        case 5: {
          if (tag !== 42) {
            break;
          }

          message.elevation = reader.string();
          continue;
        }
        case 6: {
          if (tag !== 50) {
            break;
          }

          message.harvest_time = reader.string();
          continue;
        }
        case 7: {
          if (tag !== 58) {
            break;
          }

          message.variety = reader.string();
          continue;
        }
        case 8: {
          if (tag !== 66) {
            break;
          }

          message.processing = reader.string();
          continue;
        }
        case 9: {
          if (tag !== 74) {
            break;
          }

          message.certification = reader.string();
          continue;
        }
        case 10: {
          if (tag === 80) {
            message.percentage = reader.uint32();
            continue;
          }
          if (tag !== 81) {
            break;
          }

          message.percentage = reader.double();
          continue;
        }
        case 11: {
          if (tag === 88) {
            message.purchasing_price = reader.uint32();
            continue;
          }
          if (tag !== 89) {
            break;
          }

          message.purchasing_price = reader.double();
          continue;
        }
        case 12: {
          if (tag === 96) {
            message.fob_price = reader.uint32();
            continue;
          }
          if (tag !== 97) {
            break;
          }

          message.fob_price = reader.double();
          continue;
        }
      }
      if ((tag & 7) === 4 || tag === 0) {
        break;
      }
      reader.skip(tag & 7);
    }
    return message;
  },

  fromJSON(object: any): BeanInformation {
    return {
      country: isSet(object.country)
        ? globalThis.String(object.country)
        : undefined,
      region: isSet(object.region)
        ? globalThis.String(object.region)
        : undefined,
      farm: isSet(object.farm) ? globalThis.String(object.farm) : undefined,
      farmer: isSet(object.farmer)
        ? globalThis.String(object.farmer)
        : undefined,
      elevation: isSet(object.elevation)
        ? globalThis.String(object.elevation)
        : undefined,
      harvest_time: isSet(object.harvestTime)
        ? globalThis.String(object.harvestTime)
        : isSet(object.harvest_time)
          ? globalThis.String(object.harvest_time)
          : undefined,
      variety: isSet(object.variety)
        ? globalThis.String(object.variety)
        : undefined,
      processing: isSet(object.processing)
        ? globalThis.String(object.processing)
        : undefined,
      certification: isSet(object.certification)
        ? globalThis.String(object.certification)
        : undefined,
      percentage: isSet(object.percentage)
        ? globalThis.Number(object.percentage)
        : undefined,
      purchasing_price: isSet(object.purchasingPrice)
        ? globalThis.Number(object.purchasingPrice)
        : isSet(object.purchasing_price)
          ? globalThis.Number(object.purchasing_price)
          : undefined,
      fob_price: isSet(object.fobPrice)
        ? globalThis.Number(object.fobPrice)
        : isSet(object.fob_price)
          ? globalThis.Number(object.fob_price)
          : undefined,
    };
  },

  toJSON(message: BeanInformation): unknown {
    const obj: any = {};
    if (message.country !== undefined) {
      obj.country = message.country;
    }
    if (message.region !== undefined) {
      obj.region = message.region;
    }
    if (message.farm !== undefined) {
      obj.farm = message.farm;
    }
    if (message.farmer !== undefined) {
      obj.farmer = message.farmer;
    }
    if (message.elevation !== undefined) {
      obj.elevation = message.elevation;
    }
    if (message.harvest_time !== undefined) {
      obj.harvestTime = message.harvest_time;
    }
    if (message.variety !== undefined) {
      obj.variety = message.variety;
    }
    if (message.processing !== undefined) {
      obj.processing = message.processing;
    }
    if (message.certification !== undefined) {
      obj.certification = message.certification;
    }
    if (message.percentage !== undefined) {
      obj.percentage = message.percentage;
    }
    if (message.purchasing_price !== undefined) {
      obj.purchasingPrice = message.purchasing_price;
    }
    if (message.fob_price !== undefined) {
      obj.fobPrice = message.fob_price;
    }
    return obj;
  },

  create<I extends Exact<DeepPartial<BeanInformation>, I>>(
    base?: I,
  ): BeanInformation {
    return BeanInformation.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<BeanInformation>, I>>(
    object: I,
  ): BeanInformation {
    const message = createBaseBeanInformation();
    message.country = object.country ?? undefined;
    message.region = object.region ?? undefined;
    message.farm = object.farm ?? undefined;
    message.farmer = object.farmer ?? undefined;
    message.elevation = object.elevation ?? undefined;
    message.harvest_time = object.harvest_time ?? undefined;
    message.variety = object.variety ?? undefined;
    message.processing = object.processing ?? undefined;
    message.certification = object.certification ?? undefined;
    message.percentage = object.percentage ?? undefined;
    message.purchasing_price = object.purchasing_price ?? undefined;
    message.fob_price = object.fob_price ?? undefined;
    return message;
  },
};

function createBaseBeanRoastInformation(): BeanRoastInformation {
  return {
    drop_temperature: undefined,
    roast_length: undefined,
    roaster_machine: undefined,
    green_bean_weight: undefined,
    outside_temperature: undefined,
    humidity: undefined,
    bean_uuid: undefined,
    first_crack_minute: undefined,
    first_crack_temperature: undefined,
    second_crack_minute: undefined,
    second_crack_temperature: undefined,
  };
}

export const BeanRoastInformation: MessageFns<BeanRoastInformation> = {
  encode(
    message: BeanRoastInformation,
    writer: BinaryWriter = new BinaryWriter(),
  ): BinaryWriter {
    if (message.drop_temperature !== undefined) {
      writer.uint32(9).double(message.drop_temperature);
    }
    if (message.roast_length !== undefined) {
      writer.uint32(17).double(message.roast_length);
    }
    if (message.roaster_machine !== undefined) {
      writer.uint32(26).string(message.roaster_machine);
    }
    if (message.green_bean_weight !== undefined) {
      writer.uint32(33).double(message.green_bean_weight);
    }
    if (message.outside_temperature !== undefined) {
      writer.uint32(41).double(message.outside_temperature);
    }
    if (message.humidity !== undefined) {
      writer.uint32(49).double(message.humidity);
    }
    if (message.bean_uuid !== undefined) {
      writer.uint32(58).string(message.bean_uuid);
    }
    if (message.first_crack_minute !== undefined) {
      writer.uint32(65).double(message.first_crack_minute);
    }
    if (message.first_crack_temperature !== undefined) {
      writer.uint32(73).double(message.first_crack_temperature);
    }
    if (message.second_crack_minute !== undefined) {
      writer.uint32(81).double(message.second_crack_minute);
    }
    if (message.second_crack_temperature !== undefined) {
      writer.uint32(89).double(message.second_crack_temperature);
    }
    return writer;
  },

  decode(
    input: BinaryReader | Uint8Array,
    length?: number,
  ): BeanRoastInformation {
    const reader =
      input instanceof BinaryReader ? input : new BinaryReader(input);
    const end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseBeanRoastInformation();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1: {
          if (tag === 8) {
            message.drop_temperature = reader.uint32();
            continue;
          }
          if (tag !== 9) {
            break;
          }

          message.drop_temperature = reader.double();
          continue;
        }
        case 2: {
          if (tag === 16) {
            message.roast_length = longToNumber(reader.uint64());
            continue;
          }
          if (tag !== 17) {
            break;
          }

          message.roast_length = reader.double();
          continue;
        }
        case 3: {
          if (tag !== 26) {
            break;
          }

          message.roaster_machine = reader.string();
          continue;
        }
        case 4: {
          if (tag === 32) {
            message.green_bean_weight = longToNumber(reader.uint64());
            continue;
          }
          if (tag !== 33) {
            break;
          }

          message.green_bean_weight = reader.double();
          continue;
        }
        case 5: {
          if (tag === 40) {
            message.outside_temperature = reader.uint32();
            continue;
          }
          if (tag !== 41) {
            break;
          }

          message.outside_temperature = reader.double();
          continue;
        }
        case 6: {
          if (tag === 48) {
            message.humidity = reader.uint32();
            continue;
          }
          if (tag !== 49) {
            break;
          }

          message.humidity = reader.double();
          continue;
        }
        case 7: {
          if (tag !== 58) {
            break;
          }

          message.bean_uuid = reader.string();
          continue;
        }
        case 8: {
          if (tag === 64) {
            message.first_crack_minute = reader.uint32();
            continue;
          }
          if (tag !== 65) {
            break;
          }

          message.first_crack_minute = reader.double();
          continue;
        }
        case 9: {
          if (tag === 72) {
            message.first_crack_temperature = reader.uint32();
            continue;
          }
          if (tag !== 73) {
            break;
          }

          message.first_crack_temperature = reader.double();
          continue;
        }
        case 10: {
          if (tag === 80) {
            message.second_crack_minute = reader.uint32();
            continue;
          }
          if (tag !== 81) {
            break;
          }

          message.second_crack_minute = reader.double();
          continue;
        }
        case 11: {
          if (tag === 88) {
            message.second_crack_temperature = reader.uint32();
            continue;
          }
          if (tag !== 89) {
            break;
          }

          message.second_crack_temperature = reader.double();
          continue;
        }
      }
      if ((tag & 7) === 4 || tag === 0) {
        break;
      }
      reader.skip(tag & 7);
    }
    return message;
  },

  fromJSON(object: any): BeanRoastInformation {
    return {
      drop_temperature: isSet(object.dropTemperature)
        ? globalThis.Number(object.dropTemperature)
        : isSet(object.drop_temperature)
          ? globalThis.Number(object.drop_temperature)
          : undefined,
      roast_length: isSet(object.roastLength)
        ? globalThis.Number(object.roastLength)
        : isSet(object.roast_length)
          ? globalThis.Number(object.roast_length)
          : undefined,
      roaster_machine: isSet(object.roasterMachine)
        ? globalThis.String(object.roasterMachine)
        : isSet(object.roaster_machine)
          ? globalThis.String(object.roaster_machine)
          : undefined,
      green_bean_weight: isSet(object.greenBeanWeight)
        ? globalThis.Number(object.greenBeanWeight)
        : isSet(object.green_bean_weight)
          ? globalThis.Number(object.green_bean_weight)
          : undefined,
      outside_temperature: isSet(object.outsideTemperature)
        ? globalThis.Number(object.outsideTemperature)
        : isSet(object.outside_temperature)
          ? globalThis.Number(object.outside_temperature)
          : undefined,
      humidity: isSet(object.humidity)
        ? globalThis.Number(object.humidity)
        : undefined,
      bean_uuid: isSet(object.beanUuid)
        ? globalThis.String(object.beanUuid)
        : isSet(object.bean_uuid)
          ? globalThis.String(object.bean_uuid)
          : undefined,
      first_crack_minute: isSet(object.firstCrackMinute)
        ? globalThis.Number(object.firstCrackMinute)
        : isSet(object.first_crack_minute)
          ? globalThis.Number(object.first_crack_minute)
          : undefined,
      first_crack_temperature: isSet(object.firstCrackTemperature)
        ? globalThis.Number(object.firstCrackTemperature)
        : isSet(object.first_crack_temperature)
          ? globalThis.Number(object.first_crack_temperature)
          : undefined,
      second_crack_minute: isSet(object.secondCrackMinute)
        ? globalThis.Number(object.secondCrackMinute)
        : isSet(object.second_crack_minute)
          ? globalThis.Number(object.second_crack_minute)
          : undefined,
      second_crack_temperature: isSet(object.secondCrackTemperature)
        ? globalThis.Number(object.secondCrackTemperature)
        : isSet(object.second_crack_temperature)
          ? globalThis.Number(object.second_crack_temperature)
          : undefined,
    };
  },

  toJSON(message: BeanRoastInformation): unknown {
    const obj: any = {};
    if (message.drop_temperature !== undefined) {
      obj.dropTemperature = message.drop_temperature;
    }
    if (message.roast_length !== undefined) {
      obj.roastLength = message.roast_length;
    }
    if (message.roaster_machine !== undefined) {
      obj.roasterMachine = message.roaster_machine;
    }
    if (message.green_bean_weight !== undefined) {
      obj.greenBeanWeight = message.green_bean_weight;
    }
    if (message.outside_temperature !== undefined) {
      obj.outsideTemperature = message.outside_temperature;
    }
    if (message.humidity !== undefined) {
      obj.humidity = message.humidity;
    }
    if (message.bean_uuid !== undefined) {
      obj.beanUuid = message.bean_uuid;
    }
    if (message.first_crack_minute !== undefined) {
      obj.firstCrackMinute = message.first_crack_minute;
    }
    if (message.first_crack_temperature !== undefined) {
      obj.firstCrackTemperature = message.first_crack_temperature;
    }
    if (message.second_crack_minute !== undefined) {
      obj.secondCrackMinute = message.second_crack_minute;
    }
    if (message.second_crack_temperature !== undefined) {
      obj.secondCrackTemperature = message.second_crack_temperature;
    }
    return obj;
  },

  create<I extends Exact<DeepPartial<BeanRoastInformation>, I>>(
    base?: I,
  ): BeanRoastInformation {
    return BeanRoastInformation.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<BeanRoastInformation>, I>>(
    object: I,
  ): BeanRoastInformation {
    const message = createBaseBeanRoastInformation();
    message.drop_temperature = object.drop_temperature ?? undefined;
    message.roast_length = object.roast_length ?? undefined;
    message.roaster_machine = object.roaster_machine ?? undefined;
    message.green_bean_weight = object.green_bean_weight ?? undefined;
    message.outside_temperature = object.outside_temperature ?? undefined;
    message.humidity = object.humidity ?? undefined;
    message.bean_uuid = object.bean_uuid ?? undefined;
    message.first_crack_minute = object.first_crack_minute ?? undefined;
    message.first_crack_temperature =
      object.first_crack_temperature ?? undefined;
    message.second_crack_minute = object.second_crack_minute ?? undefined;
    message.second_crack_temperature =
      object.second_crack_temperature ?? undefined;
    return message;
  },
};

function createBaseICupping(): ICupping {
  return {
    dry_fragrance: undefined,
    wet_aroma: undefined,
    brightness: undefined,
    flavor: undefined,
    body: undefined,
    finish: undefined,
    sweetness: undefined,
    clean_cup: undefined,
    complexity: undefined,
    uniformity: undefined,
    cuppers_correction: undefined,
  };
}

export const ICupping: MessageFns<ICupping> = {
  encode(
    message: ICupping,
    writer: BinaryWriter = new BinaryWriter(),
  ): BinaryWriter {
    if (message.dry_fragrance !== undefined) {
      writer.uint32(9).double(message.dry_fragrance);
    }
    if (message.wet_aroma !== undefined) {
      writer.uint32(17).double(message.wet_aroma);
    }
    if (message.brightness !== undefined) {
      writer.uint32(25).double(message.brightness);
    }
    if (message.flavor !== undefined) {
      writer.uint32(33).double(message.flavor);
    }
    if (message.body !== undefined) {
      writer.uint32(41).double(message.body);
    }
    if (message.finish !== undefined) {
      writer.uint32(49).double(message.finish);
    }
    if (message.sweetness !== undefined) {
      writer.uint32(57).double(message.sweetness);
    }
    if (message.clean_cup !== undefined) {
      writer.uint32(65).double(message.clean_cup);
    }
    if (message.complexity !== undefined) {
      writer.uint32(73).double(message.complexity);
    }
    if (message.uniformity !== undefined) {
      writer.uint32(81).double(message.uniformity);
    }
    if (message.cuppers_correction !== undefined) {
      writer.uint32(89).double(message.cuppers_correction);
    }
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): ICupping {
    const reader =
      input instanceof BinaryReader ? input : new BinaryReader(input);
    const end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseICupping();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1: {
          if (tag === 8) {
            message.dry_fragrance = longToNumber(reader.uint64());
            continue;
          }
          if (tag !== 9) {
            break;
          }

          message.dry_fragrance = reader.double();
          continue;
        }
        case 2: {
          if (tag === 16) {
            message.wet_aroma = longToNumber(reader.uint64());
            continue;
          }
          if (tag !== 17) {
            break;
          }

          message.wet_aroma = reader.double();
          continue;
        }
        case 3: {
          if (tag === 24) {
            message.brightness = longToNumber(reader.uint64());
            continue;
          }
          if (tag !== 25) {
            break;
          }

          message.brightness = reader.double();
          continue;
        }
        case 4: {
          if (tag === 32) {
            message.flavor = longToNumber(reader.uint64());
            continue;
          }
          if (tag !== 33) {
            break;
          }

          message.flavor = reader.double();
          continue;
        }
        case 5: {
          if (tag === 40) {
            message.body = longToNumber(reader.uint64());
            continue;
          }
          if (tag !== 41) {
            break;
          }

          message.body = reader.double();
          continue;
        }
        case 6: {
          if (tag === 48) {
            message.finish = longToNumber(reader.uint64());
            continue;
          }
          if (tag !== 49) {
            break;
          }

          message.finish = reader.double();
          continue;
        }
        case 7: {
          if (tag === 56) {
            message.sweetness = longToNumber(reader.uint64());
            continue;
          }
          if (tag !== 57) {
            break;
          }

          message.sweetness = reader.double();
          continue;
        }
        case 8: {
          if (tag === 64) {
            message.clean_cup = longToNumber(reader.uint64());
            continue;
          }
          if (tag !== 65) {
            break;
          }

          message.clean_cup = reader.double();
          continue;
        }
        case 9: {
          if (tag === 72) {
            message.complexity = longToNumber(reader.uint64());
            continue;
          }
          if (tag !== 73) {
            break;
          }

          message.complexity = reader.double();
          continue;
        }
        case 10: {
          if (tag === 80) {
            message.uniformity = longToNumber(reader.uint64());
            continue;
          }
          if (tag !== 81) {
            break;
          }

          message.uniformity = reader.double();
          continue;
        }
        case 11: {
          if (tag === 88) {
            message.cuppers_correction = longToNumber(reader.uint64());
            continue;
          }
          if (tag !== 89) {
            break;
          }

          message.cuppers_correction = reader.double();
          continue;
        }
      }
      if ((tag & 7) === 4 || tag === 0) {
        break;
      }
      reader.skip(tag & 7);
    }
    return message;
  },

  fromJSON(object: any): ICupping {
    return {
      dry_fragrance: isSet(object.dryFragrance)
        ? globalThis.Number(object.dryFragrance)
        : isSet(object.dry_fragrance)
          ? globalThis.Number(object.dry_fragrance)
          : undefined,
      wet_aroma: isSet(object.wetAroma)
        ? globalThis.Number(object.wetAroma)
        : isSet(object.wet_aroma)
          ? globalThis.Number(object.wet_aroma)
          : undefined,
      brightness: isSet(object.brightness)
        ? globalThis.Number(object.brightness)
        : undefined,
      flavor: isSet(object.flavor)
        ? globalThis.Number(object.flavor)
        : undefined,
      body: isSet(object.body) ? globalThis.Number(object.body) : undefined,
      finish: isSet(object.finish)
        ? globalThis.Number(object.finish)
        : undefined,
      sweetness: isSet(object.sweetness)
        ? globalThis.Number(object.sweetness)
        : undefined,
      clean_cup: isSet(object.cleanCup)
        ? globalThis.Number(object.cleanCup)
        : isSet(object.clean_cup)
          ? globalThis.Number(object.clean_cup)
          : undefined,
      complexity: isSet(object.complexity)
        ? globalThis.Number(object.complexity)
        : undefined,
      uniformity: isSet(object.uniformity)
        ? globalThis.Number(object.uniformity)
        : undefined,
      cuppers_correction: isSet(object.cuppersCorrection)
        ? globalThis.Number(object.cuppersCorrection)
        : isSet(object.cuppers_correction)
          ? globalThis.Number(object.cuppers_correction)
          : undefined,
    };
  },

  toJSON(message: ICupping): unknown {
    const obj: any = {};
    if (message.dry_fragrance !== undefined) {
      obj.dryFragrance = message.dry_fragrance;
    }
    if (message.wet_aroma !== undefined) {
      obj.wetAroma = message.wet_aroma;
    }
    if (message.brightness !== undefined) {
      obj.brightness = message.brightness;
    }
    if (message.flavor !== undefined) {
      obj.flavor = message.flavor;
    }
    if (message.body !== undefined) {
      obj.body = message.body;
    }
    if (message.finish !== undefined) {
      obj.finish = message.finish;
    }
    if (message.sweetness !== undefined) {
      obj.sweetness = message.sweetness;
    }
    if (message.clean_cup !== undefined) {
      obj.cleanCup = message.clean_cup;
    }
    if (message.complexity !== undefined) {
      obj.complexity = message.complexity;
    }
    if (message.uniformity !== undefined) {
      obj.uniformity = message.uniformity;
    }
    if (message.cuppers_correction !== undefined) {
      obj.cuppersCorrection = message.cuppers_correction;
    }
    return obj;
  },

  create<I extends Exact<DeepPartial<ICupping>, I>>(base?: I): ICupping {
    return ICupping.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<ICupping>, I>>(object: I): ICupping {
    const message = createBaseICupping();
    message.dry_fragrance = object.dry_fragrance ?? undefined;
    message.wet_aroma = object.wet_aroma ?? undefined;
    message.brightness = object.brightness ?? undefined;
    message.flavor = object.flavor ?? undefined;
    message.body = object.body ?? undefined;
    message.finish = object.finish ?? undefined;
    message.sweetness = object.sweetness ?? undefined;
    message.clean_cup = object.clean_cup ?? undefined;
    message.complexity = object.complexity ?? undefined;
    message.uniformity = object.uniformity ?? undefined;
    message.cuppers_correction = object.cuppers_correction ?? undefined;
    return message;
  },
};

function createBaseIFlavor(): IFlavor {
  return { predefined_flavors: [], custom_flavors: [] };
}

export const IFlavor: MessageFns<IFlavor> = {
  encode(
    message: IFlavor,
    writer: BinaryWriter = new BinaryWriter(),
  ): BinaryWriter {
    writer.uint32(10).fork();
    for (const v of message.predefined_flavors) {
      writer.uint64(v);
    }
    writer.join();
    for (const v of message.custom_flavors) {
      writer.uint32(18).string(v!);
    }
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): IFlavor {
    const reader =
      input instanceof BinaryReader ? input : new BinaryReader(input);
    const end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseIFlavor();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1: {
          if (tag === 8) {
            message.predefined_flavors.push(longToNumber(reader.uint64()));

            continue;
          }

          if (tag === 10) {
            const end2 = reader.uint32() + reader.pos;
            while (reader.pos < end2) {
              message.predefined_flavors.push(longToNumber(reader.uint64()));
            }

            continue;
          }

          break;
        }
        case 2: {
          if (tag !== 18) {
            break;
          }

          message.custom_flavors.push(reader.string());
          continue;
        }
      }
      if ((tag & 7) === 4 || tag === 0) {
        break;
      }
      reader.skip(tag & 7);
    }
    return message;
  },

  fromJSON(object: any): IFlavor {
    return {
      predefined_flavors: globalThis.Array.isArray(object?.predefinedFlavors)
        ? object.predefinedFlavors.map((e: any) => globalThis.Number(e))
        : globalThis.Array.isArray(object?.predefined_flavors)
          ? object.predefined_flavors.map((e: any) => globalThis.Number(e))
          : [],
      custom_flavors: globalThis.Array.isArray(object?.customFlavors)
        ? object.customFlavors.map((e: any) => globalThis.String(e))
        : globalThis.Array.isArray(object?.custom_flavors)
          ? object.custom_flavors.map((e: any) => globalThis.String(e))
          : [],
    };
  },

  toJSON(message: IFlavor): unknown {
    const obj: any = {};
    if (message.predefined_flavors?.length) {
      obj.predefinedFlavors = message.predefined_flavors.map((e) =>
        Math.round(e),
      );
    }
    if (message.custom_flavors?.length) {
      obj.customFlavors = message.custom_flavors;
    }
    return obj;
  },

  create<I extends Exact<DeepPartial<IFlavor>, I>>(base?: I): IFlavor {
    return IFlavor.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<IFlavor>, I>>(object: I): IFlavor {
    const message = createBaseIFlavor();
    message.predefined_flavors = object.predefined_flavors?.map((e) => e) || [];
    message.custom_flavors = object.custom_flavors?.map((e) => e) || [];
    return message;
  },
};

type Builtin =
  | Date
  | Function
  | Uint8Array
  | string
  | number
  | boolean
  | undefined;

export type DeepPartial<T> = T extends Builtin
  ? T
  : T extends globalThis.Array<infer U>
    ? globalThis.Array<DeepPartial<U>>
    : T extends ReadonlyArray<infer U>
      ? ReadonlyArray<DeepPartial<U>>
      : T extends {}
        ? { [K in keyof T]?: DeepPartial<T[K]> }
        : Partial<T>;

type KeysOfUnion<T> = T extends T ? keyof T : never;
export type Exact<P, I extends P> = P extends Builtin
  ? P
  : P & { [K in keyof P]: Exact<P[K], I[K]> } & {
      [K in Exclude<keyof I, KeysOfUnion<P>>]: never;
    };

function longToNumber(int64: { toString(): string }): number {
  const num = globalThis.Number(int64.toString());
  if (num > globalThis.Number.MAX_SAFE_INTEGER) {
    throw new globalThis.Error('Value is larger than Number.MAX_SAFE_INTEGER');
  }
  if (num < globalThis.Number.MIN_SAFE_INTEGER) {
    throw new globalThis.Error('Value is smaller than Number.MIN_SAFE_INTEGER');
  }
  return num;
}

function isSet(value: any): boolean {
  return value !== null && value !== undefined;
}

export interface MessageFns<T> {
  encode(message: T, writer?: BinaryWriter): BinaryWriter;
  decode(input: BinaryReader | Uint8Array, length?: number): T;
  fromJSON(object: any): T;
  toJSON(message: T): unknown;
  create<I extends Exact<DeepPartial<T>, I>>(base?: I): T;
  fromPartial<I extends Exact<DeepPartial<T>, I>>(object: I): T;
}
